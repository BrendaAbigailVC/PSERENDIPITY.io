...
def ruta_chat(mysql, session):
    ...
    conversaciones = obtener_conversaciones(mysql,session)
    return render_template('chatS.html', mensajes_clasificados=mensajes_clasificados, conversaciones=conversaciones)


def buscar_usuario_por_correo(mysql, correo):
    cur = mysql.cursor()
...
def buscar_e_iniciar_chat(mysql, session, request):
    correo = request.form['correo']
    usuario = buscar_usuario_por_correo(mysql, correo)
    if usuario:
        return iniciar_conversacion(mysql, session, usuario['id'])
    else:
        return "Usuario no encontrado"

def iniciar_conversacion(mysql, session, id_usuario_destino):
    if not is_user_logged_in(session):
        return redirect(url_for('login'))
    ...
    """, (usuario_id, id_usuario_destino, id_usuario_destino, usuario_id))
    
    conversacion = cur.fetchone()

    if not conversacion:
        cur.execute("""
            INSERT INTO conversaciones (usuario1_id, usuario2_id, fecha_inicio) 
            VALUES (%s, %s, %s)
        """, (usuario_id, id_usuario_destino, datetime.datetime.now()))
        mysql.commit()
    ...
    cur.close()
    return redirect(url_for('ver_conversacion_route', conversacion_id=conversacion_id))

def ver_conversacion(mysql, session, conversacion_id):
    if not is_user_logged_in(session):
        return redirect(url_for('login'))

    cur = mysql.cursor()
    cur.execute("SELECT * FROM conversaciones WHERE id = %s", (conversacion_id,))
    conversacion = cur.fetchone()
    ...
    cur.execute("SELECT mensajes.*, usuarios.name AS nombre FROM mensajes INNER JOIN usuarios ON mensajes.usuario_id = usuarios.id WHERE conversacion_id = %s ORDER BY fecha_envio ASC", (conversacion_id,))
    ...
    for mensaje in mensajes:
        if mensaje['usuario_id'] == usuario_id:
            mensajes_clasificados['mis_mensajes'].append(mensaje)
        else:
            mensajes_clasificados['mensajes_otros'].append(mensaje)
    
    conversaciones = obtener_conversaciones(mysql, session)
    
    return render_template('chatS.html', mensajes_clasificados=mensajes_clasificados, conversacion_id=conversacion_id, conversaciones=conversaciones)

def ruta_enviar_mensaje(mysql, session, request):
    if request.method == 'POST':
        mensaje = request.form['mensaje']
        conversacion_id = request.form['conversacion_id']  
        if validar_mensaje(mysql, mensaje):  # Pasar el mensaje como argumento a validar_mensaje()
            cur = mysql.cursor()
            cur.execute("INSERT INTO mensajes (conversacion_id, usuario_id, contenido, fecha_envio) VALUES (%s, %s, %s, %s)",
                        (conversacion_id, session['id'], mensaje, datetime.datetime.now()))
            mysql.commit()
            cur.close()
    return redirect(url_for('ver_conversacion_route', conversacion_id=conversacion_id))


def obtener_mensajes(mysql):
    cur = mysql.cursor()
    cur.execute("SELECT * FROM mensajes ORDER BY fecha_envio DESC LIMIT 50")
    mensajes = cur.fetchall()
    cur.close()
    return mensajes

def validar_mensaje(mysql, mensaje):
    return mensaje.strip() != ''

def obtener_conversaciones(mysql, session):
    ...
    cur.execute("""
            ...
            c.usuario1_id, 
            c.usuario2_id, 
            u1.name as usuario1_nombre, 
            u1.avatar as usuario1_avatar,
            u2.name as usuario2_nombre, 
    ...
    for conversacion in conversaciones:
        if conversacion['usuario1_id'] == usuario_id:
            otro_usuario = {
                'nombre': conversacion['usuario2_nombre'],
                'avatar': conversacion['usuario2_avatar']
            }
            if otro_usuario['avatar']:
                try:
                    encoded_avatar = base64.b64encode(otro_usuario['avatar']).decode('utf-8')
                    otro_usuario['avatar'] = f'data:image/jpeg;base64,{encoded_avatar}'
                except Exception as e:
                    otro_usuario['avatar'] = f'Error al cargar el avatar: {str(e)}'
        else:
            otro_usuario = {
                'nombre': conversacion['usuario1_nombre'],
                'avatar': conversacion['usuario1_avatar']
            }
            if otro_usuario['avatar']:
                try:
                    encoded_avatar = base64.b64encode(otro_usuario['avatar']).decode('utf-8')
